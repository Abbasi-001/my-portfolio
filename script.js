...
function initServiceConnectors() {
  const svg = document.getElementById('servicesConnectors');
  ...
  function drawLines() {
    ...
    // Connect adjacent cards with animated lines
    for (let i = 0; i < cards.length - 1; i++) {
      if (cards[i].classList.contains('services-row-divider')) continue;
      ...
      // Only draw horizontal connections (same row approx)
      if (Math.abs(y1 - y2) > 80) continue;

      const lineId = 'svcLine' + i;
      const gradId = 'svcGrad' + i;
      ...
      s1.setAttribute('offset','0%'); s1.setAttribute('stop-color','rgba(255,30,30,0)');
      const s2 = document.createElementNS('http://www.w3.org/2000/svg','stop');
      s2.setAttribute('offset','50%'); s2.setAttribute('stop-color','rgba(255,30,30,0.5)');
      const s3 = document.createElementNS('http://www.w3.org/2000/svg','stop');
      s3.setAttribute('offset','100%'); s3.setAttribute('stop-color','rgba(255,30,30,0)');
      ...
      animate.setAttribute('from','0'); animate.setAttribute('to','40');
